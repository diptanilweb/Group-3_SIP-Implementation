...
void TcpClient::sendmsgtotcpserver()
{
	while(1)
	{
		...
		else
		{
			...
			cout<<tserver_msg<<endl;
			//to store the log details of datas transfer between user agents
			auto time1=chrono::system_clock::to_time_t(chrono::system_clock::now());
			f<<"The message received from the USER AGENT SERVER is: "<<tserver_msg<<" at time:"<<ctime(&time1)<<endl;
		}
		...
		else
		{
			...
			f<<"The message send to the USER AGENT SERVER is: "<<tclient_msg<<" at time:"<<ctime(&time2)<<endl;
		}

...
void TcpServer::sendmsgtotcpclient()
{
	while(1)
	{
		...
		else
		{
			...
			cout<<tserver_msg<<endl;
			//to store the log details of data transfer between user agents
			auto time3 = chrono::system_clock::to_time_t(chrono::system_clock::now());
			f<<"The message received from the USER AGENT CLIENT is: "<<tserver_msg<<" at time:"<<ctime(&time3)<<endl;
		}
		...
		else
		{
			...
			//to store the log details of data transfer between user agents
			auto time4 = chrono::system_clock::to_time_t(chrono::system_clock::now());
			f<<"The message send to the USER AGENT CLIENT is: "<<tclient_msg<<" at time:"<<ctime(&time4)<<endl;
		}
